...
const MakeInvoice = () => {
  const { customers, orders, addOrder, setCustomers, updateCustomer, catalog, setCatalog, selectedBranch, payments, setPayments, services, addCustomer, areas } = useContext(AdminStateContext);
  ...
  const handleServiceModeChange = (newService) => {
    setQuickServiceMode(newService);
    setOrderItems(prev => prev.map(item => {
      const isCarpet = String(item.name || '').toLowerCase().includes('carpet') || String(item.nameAr || '').includes('سجاد');
      const originalGarment = catalog.find(g => g.name === item.name);
      
      let newPrice = item.unitPrice;
      // Auto-update price for normal items based on the new service
      if (originalGarment && !isCarpet && !originalGarment.hasSizes) {
         newPrice = originalGarment.customPrice !== undefined ? originalGarment.customPrice : getGarmentPriceForService(originalGarment, newService);
      }
                  ...
                  {(() => {
                      ...
                      return sorted.map((service) => (
                          <button
                              key={service.id}
                              type="button"
                              onClick={() => handleServiceModeChange(service.name)}
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-surface-alt/75 text-secondary border-b border-border/80 font-bold">
                    <th className="px-2 py-1.5 w-5/12">{t('counter.makeInvoice.itemServiceHeader') || "Item / Service"}</th>
                    <th className="px-2 py-1.5 w-3/12 text-center">{t('counter.makeInvoice.qtyHeader') || "Qty"}</th>
